...
**Testing note:** `backfill_history` takes a `min_rows` parameter defaulting to
`SPARSE_ROW_THRESHOLD`. Without it, a compact test fixture is indistinguishable
from a weekend and every fetch-count test fails. A test asserts the default
equals the production threshold, so the override cannot silently weaken
weekend rejection.
